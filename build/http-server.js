...
class FeishuClient {
    ...
    // 下载文档
    async downloadDocument(fileToken) {
        try {
            const token = await this.getAccessToken();
            // 首先获取文档元数据
    ...
    // 下载空间内所有文档
    async downloadSpaceDocuments(spaceId) {
        try {
            // 获取空间内所有文档
                    ...
                    content: [
                        {
                            ...
                            text: `找到 ${spaces.length} 个文档空间:\n\n` +
                                spaces.map((space, index) => `${index + 1}. ${space.name} (ID: ${space.space_id})`).join('\n')
                        }
                    ]
                };
            case 'list-documents':
                    ...
                    content: [
                        {
                            ...
                            text: `找到 ${documents.length} 个文档:\n\n` +
                                documents.map((doc, index) => `${index + 1}. ${doc.title} (ID: ${doc.node_token})`).join('\n')
                        }
                    ]
                };
...
server.listen(port, '0.0.0.0', () => {
    ...
    console.log(`请在Cursor中配置使用地址: http://localhost:${address.port}/mcp`);
    console.log(`您也可以在浏览器中访问 http://localhost:${address.port} 查看服务器状态`);
});
// 处理服务器错误
