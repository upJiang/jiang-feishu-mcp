...
// Define simplified McpServer type
interface McpServer {
  tool<T = any>(
    name: string,
...
export function registerTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ spaceId }: { spaceId?: string }) => {
      try {
        ...
        // 按空间分组
        const docsBySpace: Record<string, Array<DocItem>> = {};
  ...
  server.tool(
    ...
    async ({ fileToken, name, type }: { fileToken: string, name?: string, type?: string }) => {
      try {
  ...
  server.tool(
    ...
    async ({ spaceId }: { spaceId?: string }) => {
      try {
        ...
        const results: Array<{name: string, success: boolean, path?: string, error?: string}> = [];
  ...
  server.tool(
    ...
    async ({ query }: { query: string }) => {
      try {
        ...
        // 按空间分组
        const docsBySpace: Record<string, typeof docs> = {};
